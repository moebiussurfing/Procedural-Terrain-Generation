...
void GUI::random() {
	...
	seedSlider = ofRandom(seedSlider.getMin(), seedSlider.getMax());
}
...
GUI::GUI() {
	...
	palette.resize(NUM_COLOR);

	// subscribe palette to auto update
	colorClient.setLinkPalette(palette);

	// we can subscribe the target background color to allow auto-update
	colorClient.setLinkColorBg(colorBackGround);
#endif
...
#ifdef USE_COLOR_CLIENT
	int i = 0;
	oceanColorPicker = palette[i++];
	coastColorPicker = palette[i++];
	beachColorPicker = palette[i++];
	plainColorPicker = palette[i++];

	//int sz = palette.size();
	//if (sz > 4) forestColorPicker = palette[i++];
	//else i = 0;
	//if (sz > 5) hillColorPicker = palette[i++];
	//else i = 0;
	//if (sz > 6) mountainColorPicker = palette[i++];
	//else i = 0;

	// clear bg
	//ofClear(colorBackGround);

	// gui debug
	colorClient.drawGui();

	// live palette and gradient editor preview 
	colorClient.draw();
#endif
}
